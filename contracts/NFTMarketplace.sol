...
contract NFTMarketplace is ReentrancyGuard
{
  ...
  // VARIABLES
  // mapping(abi.encodePacked(nftAddress, tokenid);s => Listing)) private s_listings; 
  mapping(bytes => Listing) private s_listings;
  ...
  {  // modifier checking to see if nft hasn't been listed already
    Listing memory listing = s_listings[abi.encodePacked(nftAddress_, tokenId_)];
  ...
  {  // nftAddress_ is the nft contract address
    ...
    // IERC721 iNFT = IERC721(nftAddress_); 
    ERC721 NFT = ERC721(nftAddress_);
    string memory tokenURI = NFT.tokenURI(tokenId_);
    ...
    s_listings[abi.encodePacked(nftAddress_, tokenId_)] = Listing(price_, msg.sender);
    emit ItemListed(msg.sender, nftAddress_, tokenId_, price_, NFT.name(), NFT.symbol(), tokenURI);
  }
  ...
  nonReentrant
  {
    Listing memory listedItem = s_listings[abi.encodePacked(nftAddress_, tokenId_)];
    ...
    delete(s_listings[abi.encodePacked(nftAddress_, tokenId_)]);
  ...
  isListed(nftAddress_, tokenId_)
  {
    s_listings[abi.encodePacked(nftAddress_, tokenId_)].price = newPrice_;
    ERC721 NFT = ERC721(nftAddress_);
    string memory tokenURI = NFT.tokenURI(tokenId_);
    emit ItemListed(msg.sender, nftAddress_, tokenId_, newPrice_, NFT.name(), NFT.symbol(), tokenURI);
  }
  ...
  {
    return s_listings[abi.encodePacked(nftAddress_, tokenId_)];
  }
